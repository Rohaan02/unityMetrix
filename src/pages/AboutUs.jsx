    ...
    <div>
      <section className="bg-[#0f172a] text-white py-24">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <p className="text-[#16a34a] font-semibold uppercase tracking-widest text-sm mb-3">Our Story</p>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 grid lg:grid-cols-2 gap-16 items-center">
          <div>
            <p className="text-[#16a34a] font-semibold uppercase tracking-widest text-sm mb-3">Who We Are</p>
            <h2 className="text-4xl font-black text-[#0f172a] mb-6">The Perfect Partner for <span className="text-[#16a34a]">Complete Cyber Protection</span></h2>
            ...
            <div className="grid grid-cols-3 gap-4 mb-8">
              {[["30+","Cybersecurity Projects"],["100%","Service Guarantee"],["10+","Team Experts"]].map(([n,l]) => (
                <div key={l} className="text-center bg-[#f0fdf4] rounded-2xl p-4 border border-[#16a34a30]">
                  <div className="text-2xl font-black text-[#16a34a]">{n}</div>
                  <div className="text-xs text-gray-500 mt-1">{l}</div>
                </div>
              ))}
            </div>
            <Button onClick={() => navigate("contact")} className="bg-[#16a34a] hover:bg-[#15803d] text-white rounded-full px-8 py-3 font-bold transition-colors">Get in Touch</Button>
          </div>
          <div className="bg-gradient-to-br from-[#0f172a] to-[#1e3a5f] rounded-3xl p-12 text-white text-center">
            <div className="w-28 h-28 mx-auto mb-6 relative flex items-center justify-center">
              ...
              <div className="absolute inset-9 rounded-full bg-[#16a34a] flex items-center justify-center">
                <span className="text-2xl font-black text-white">R</span>
              </div>
            </div>
            <h3 className="text-2xl font-black text-white mb-1">RISLIX</h3>
            <p className="text-[#16a34a] font-semibold mb-4 text-sm">Cyber · Privacy · GRC · AI</p>
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {values.map((v,i) => (
              <div key={i} className="bg-white rounded-2xl p-8 shadow-md border border-gray-100 hover:shadow-xl hover:border-[#16a34a40] transition-all">
                <div className="w-14 h-14 bg-[#f0fdf4] rounded-xl flex items-center justify-center mb-4 text-2xl">{v.icon}</div>
                <h3 className="font-bold text-xl text-[#0f172a] mb-3">{v.title}</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {team.map((m,i) => (
              <div key={i} className="text-center p-6 bg-gray-50 rounded-2xl hover:shadow-lg transition-shadow border border-gray-100">
                <div className="w-20 h-20 bg-gradient-to-br from-[#0f172a] to-[#1e3a5f] rounded-full mx-auto mb-4 flex items-center justify-center text-3xl">👤</div>
                <h3 className="font-bold text-[#0f172a]">{m.name}</h3>
                <p className="text-[#16a34a] text-sm font-semibold mb-2">{m.role}</p>
      ...
      <section className="py-16 bg-[#0f172a] text-white">
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <Button onClick={() => navigate("contact")} className="bg-[#16a34a] hover:bg-[#15803d] text-white rounded-full px-8 py-3 font-bold transition-colors">Work With Us</Button>
        </div>
      </section>
    </div>
  );
