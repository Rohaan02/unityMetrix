    ...
    <div>
      <section className="bg-[#0d1b2a] text-white py-24">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <p className="text-[#f5a623] font-semibold uppercase tracking-widest text-sm mb-3">Transparent Pricing</p>
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {plans.map((plan,i) => (
              <div key={i} className={`bg-white rounded-2xl border-2 overflow-hidden relative ${plan.highlighted?"border-[#f5a623] shadow-2xl shadow-[#f5a62320] scale-105":"border-gray-100 shadow-md"}`}>
                {plan.badge && <div className={`absolute top-0 right-0 text-white text-xs font-bold px-4 py-1.5 rounded-bl-xl ${plan.highlighted?"bg-[#f5a623]":"bg-[#0d1b2a]"}`}>{plan.badge}</div>}
                <div className={`p-8 ${plan.highlighted?"bg-gradient-to-br from-[#0d1b2a] to-[#1a2f45] text-white":""}`}>
                  <h3 className={`font-black text-xl mb-3 ${plan.highlighted?"text-white":"text-[#0d1b2a]"}`}>{plan.name}</h3>
                  <div className="flex items-end gap-1 mb-1">
                    <span className={`text-4xl font-black ${plan.highlighted?"text-[#f5a623]":"text-[#0d1b2a]"}`}>{plan.price}</span>
                ...
                <div className="p-8">
                  ...
                  <button onClick={() => navigate("contact")} className={`w-full py-3 rounded-full font-bold transition-colors ${plan.highlighted?"bg-[#f5a623] text-white hover:bg-[#e09415]":"border-2 border-[#f5a623] text-[#f5a623] hover:bg-[#f5a623] hover:text-white"}`}>{plan.cta}</button>
                </div>
              </div>
            ))}
          </div>
          <p className="text-center text-gray-500 text-sm mt-8">All prices are indicative. Final pricing depends on organization size, scope, and jurisdiction. <button onClick={() => navigate("contact")} className="text-[#f5a623] font-semibold hover:underline">Contact us</button> or email <a href="mailto:info@rislix.com" className="text-[#f5a623] font-semibold hover:underline">info@rislix.com</a> for a tailored quote.</p>
        </div>
      </section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[["🔍","Transparent Pricing","No hidden fees ever"],["🏆","Expert Team","Certified practitioners"],["🌍","Global Coverage","GCC, UK, EU, US"],["⚡","Fast Engagement","Start within 1–2 weeks"]].map(([icon,title,desc]) => (
              <div key={title} className="text-center p-6 bg-gray-50 rounded-2xl border border-gray-100">
                ...
                <h3 className="font-bold text-[#0d1b2a] mb-1">{title}</h3>
      ...
      <section className="py-16 bg-[#f5a623] text-white">
        <div className="max-w-2xl mx-auto px-4 text-center">
          ...
          <Button onClick={() => navigate("contact")} className="bg-[#0d1b2a] text-white hover:bg-black rounded-full px-8 py-3 font-bold transition-colors">Book Free Consultation</Button>
        </div>
      </section>
    </div>
  );
